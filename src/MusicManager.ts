...
import Discord, {Message, MessageActionRow, MessageButton, TextChannel} from "discord.js";
...
export default class MusicManager {
    ...
    addToQueue(message: Message, args: string[]) {
        ...
        this.queue.push(new Music(name, null));
        message.react("✅");
    }

    addToQueueObject(video: Music) {
    ...
    async nextSong() {
        ...
        let song: Music = this.queue.shift()!;
    ...
    stop() {
        ...
        getVoiceConnection(this.phoenixGuild.guildId)?.destroy();
    }

    async displayStatusMessage() {
        if (this.textChannel == null)
            throw new Error('No text channel assigned');
